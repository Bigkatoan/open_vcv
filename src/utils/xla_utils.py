...
def xla_print(*args):
    """Print chỉ từ master core (tránh 8 cores print cùng lúc)."""
    if _XLA_AVAILABLE:
...
Cách dùng:
    from src.utils.xla_utils import get_device, xla_available, mark_step, optimizer_step

    device = get_device()          # trả về xla:0 (TPU) hoặc cuda / cpu
    model  = model.to(device)
    ...
    loss.backward()
    optimizer_step(optimizer, scaler=None)   # tự xử lý xm.optimizer_step vs scaler.step
    mark_step()                    # BẮT BUỘC gọi sau mỗi optimizer step trên TPU

Notes:
    - torch_xla chỉ available trên Kaggle TPU / Google Cloud TPU VM.
    - Trên GPU/CPU, tất cả functions fallback về behavior thông thường.
    - TPU không hỗ trợ AMP (torch.amp.autocast) — tự động disable.
    - TPU không hỗ trợ GradScaler — scaler=None được enforce.
    - DataLoader trên TPU: dùng MpDeviceLoader từ torch_xla để pipeline data.
    - channels_last KHÔNG hỗ trợ trên TPU — tự động skip.
"""
...
def get_device() -> torch.device:
    """
    Trả về device theo thứ tự ưu tiên: TPU → CUDA → CPU.
    Trên Kaggle TPU: trả về xm.xla_device() (xla:0 hoặc xla:1..7).
    """
    if _XLA_AVAILABLE:
        return xm.xla_device()
    if torch.cuda.is_available():
...
def device_str() -> str:
    """Trả về string tên device để dùng trong argparse / config."""
    if _XLA_AVAILABLE:
        return 'xla'
    if torch.cuda.is_available():
        return 'cuda'
...
def optimizer_step(optimizer, scaler=None):
    """
    Thực hiện optimizer step đúng cách trên TPU / GPU / CPU.

    TPU:  xm.optimizer_step(optimizer) — bắt buộc dùng XLA version
    GPU:  scaler.step(optimizer) nếu dùng AMP, else optimizer.step()
    """
    if _XLA_AVAILABLE:
        xm.optimizer_step(optimizer)
    elif scaler is not None:
...
def clip_and_step(optimizer, model, scaler=None, max_norm: float = 0.5):
    """
    Gradient clip + optimizer step + mark_step (TPU) / scaler.update (GPU).
    Gộp lại để trainer.py không cần phân biệt TPU/GPU.
    """
    if _XLA_AVAILABLE:
        # TPU: unscale không cần, XLA tự handle
        torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm)
        xm.optimizer_step(optimizer)
        xm.mark_step()
    elif scaler is not None:
...
def wrap_dataloader(loader, device):
    """
    Wrap DataLoader với MpDeviceLoader trên TPU để prefetch to XLA device.
    Trên GPU/CPU: trả về loader nguyên.
    """
    if _XLA_AVAILABLE:
        return pl.MpDeviceLoader(loader, device)
...
def all_reduce_mean(tensor: torch.Tensor) -> torch.Tensor:
    """
    Trên multi-core TPU (v3-8 có 8 cores): reduce tensor across all cores.
    Trên GPU single: no-op.
    """
    if _XLA_AVAILABLE:
        return xm.all_reduce(xm.REDUCE_SUM, tensor) / xm.xrt_world_size()
    return tensor
...
def xla_print(*args):
    """Print an toàn trên TPU multi-core: chỉ print từ master core."""
    if _XLA_AVAILABLE:
        if xm.is_master_ordinal():
            print(*args)
    else:
